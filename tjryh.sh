...
#1.1.1【紧急】web 服务器目录遍历漏洞  1.1.2【中危】不存在的页面 (404) 导致物理路径泄露 1.1.3【中危】Web 服务器通用类 Cookie 注入漏洞
systemctl stop syncoverycl.service 
systemctl disable syncoverycl.service
systemctl mask syncoverycl.service
#1.1.4【中危】SSL证书不被信任 1.1.5【中危】SSL自签名证书 (无购买SSL证书意向)

#1.1.6【中危】TLS版本1.0协议检测
...
main(){
	...
	chmod 777 /home/data/
    echo "`date +"%Y-%m-%d %H:%M:%S"`  [INFO]: 加固完成，请重启主机"
}
backup_config
main
