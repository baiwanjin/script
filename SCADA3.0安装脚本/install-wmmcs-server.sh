...
#     cat > $WorkingDirectory/monitor-backend.sh<<-EOF
# #!/bin/bash
...
# User=root
# Group=windeyapps
# ExecStart=$WorkingDirectory/monitor-backend.sh
...
#     chmod +x $WorkingDirectory/monitor-backend.sh
...
EOF
    ...
    sudo firewall-cmd --add-port=9162/tcp --permanent
    sudo firewall-cmd --reload
    chmod +x windbes.service
    chmod +x $WorkingDirectory/windbes.sh
    sudo \cp windbes.service /usr/lib/systemd/system/
    sudo systemctl daemon-reload
