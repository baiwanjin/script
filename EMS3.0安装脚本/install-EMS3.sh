...
init_Chrome(){
    ...
    if [ -d "/root/桌面" ];then
        ...
        sudo chmod +x /root/桌面/windmanager.desktop
    fi
...
install_windmanager(){
    if test -d $EMS3_HOME/windmanager
    then
        sudo mv $EMS3_HOME/windmanager $EMS3_HOME/windmanager_$DATE.bak
        log_info "存在$EMS3_HOME/windmanager目录，重命名为$EMS3_HOME/windmanager_$DATE.bak"
    fi
    sudo mkdir -p $EMS3_HOME
    \cp -r windmanager $EMS3_HOME
    init_jar 'windmanager.jar' "$EMS3_HOME/windmanager" 'windmanager'
#     sed -i '/\[Service\]/a\Environment="DISPLAY=:0.0"' /usr/lib/systemd/system/windmanager.service
#     cat > /usr/lib/systemd/system/windmanager.timer <<EOF
# [Unit]
# Description=java project

# [Timer]
# OnBootSec=1min
# Unit=windmanager.service
...
#     sudo sed -i '/autologin-user=root/d' /etc/lightdm/lightdm.conf
#     sudo sed -i "/greeter-hide-users=true/a autologin-user=root" /etc/lightdm/lightdm.conf
#     sudo systemctl daemon-reload && sudo systemctl disable windmanager.service && sudo systemctl enable windmanager.timer && sudo systemctl start windmanager.service
    sudo systemctl daemon-reload && sudo systemctl enable windmanager.service && sudo systemctl start windmanager.service
}
