...
TcpPorts=(21 22 80 501 502 503 504 505 506 507 508 509 510 511 512 513 514 515 516 517 518 519 520 521 522 523 524 525 987 1433 1601 1625 1640 3306 5900 6379 8080 8082 48898 48899 8086 8182 8184 8185 9166 27017 2404 2405 2406 2407 2408 2409 2410 2411 2412 2413 2414 2415 2416 2417 2418 2419 2420 2421 2422 2423 2425 2426 2427 2428 2429 2430 2431 2432 2433 2434 2435 2436 2437 2438 2439 2440 2441 2442 2443 2444 2445 2446 2447 2448 2449 2450 2451 2452 2453 2454 2455 2456 2457 2458 2459 2460 2461 2462 2463 2464 2465 2466 2467 2468 2469 2470 2471 2472 2473 2474 2475 2476 2477 2478 2479 2480 2481 2482 2483 2484 2485
 2486 2487 2488 2489 2490 2491 2492 2493 2494 2495 2496 2497 2498 2499 2500 2501 2502 2503 2504 5866 5432)
#需要开放的UDP端口
...
EOF
    ...
    chmod +x setpasswd.sh && mv setpasswd.sh $CrontabList
    crontab -l  >/dev/null 2>&1 >> crontabfile
    if ! grep $CrontabList'/setpasswd.sh' crontabfile >/dev/null 2>&1;then echo '50 23 28-31 2,4,3,8,10,12 * '$CrontabList'/setpasswd.sh' >> crontabfile;fi
...
check_Vsftp(){
    ...
    echo "`date +"%Y-%m-%d %H:%M:%S"`  [INFO]: 未安装VSFTP"|tee -a precautions_`date +"%Y%m%d"`.log
...
#删除或禁用系统无关的用户
drop_UnusedUser(){
    UnusedUser=`cat /etc/passwd |grep "/bin/bash"|grep -v "root\|sysadm\|daemon\|adm\|lp\|sync\|shutdown\|halt\|mail\|uucp\|operator\|gopher\|ftp\|nobody\|rpm\|nscd\|avahi\|mailnull\|smmsp\|vcsa\|rpc\|sshd\|rpcuser\|nfsnobody\|pcap\|ntp\|haldaemon\|xfs\|gdm\|secadm\|audadm"|awk -F ":" '{print $1}'`
    if [[ $UnusedUser != "" ]]
    then
...
#关闭不必要的服务
stop_service(){
    servicesList=( samba proftpd inetd nfs-server.service  nfs.service Web FTP telnet rlogin NetBIOS snmpd.service )
...
get_Iptables(){
    ...
    do
        ...
        if [ $port_status == 'no' ]
        then
            echo "`date +"%Y-%m-%d %H:%M:%S"`  [INFO]: 添加UDP端口:${TcpPort}到防火墙"|tee -a precautions_`date +"%Y%m%d"`.log
...
EOF
      cp audit /etc/logrotate.d/audit
}
